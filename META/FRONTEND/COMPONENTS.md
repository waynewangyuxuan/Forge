...
项目列表中的卡片。支持活跃和非活跃（Inactive）两种状态。
...
interface ProjectCardProps {
  project: Project          // 包含 hasLocalFiles 标记
...
| 状态 | 条件 | 表现 |
|------|------|------|
| Active | `hasLocalFiles !== false` | 正常显示，可点击打开 |
| Inactive | `hasLocalFiles === false` | 半透明，显示 "Inactive" Badge，显示 "Activate" 按钮 |

**Inactive 状态**：
- 项目存在于数据库但本地文件已删除
- 显示 amber 色 "Activate" 按钮，点击从 GitHub 重新克隆
- 卡片整体 `opacity-60`，不可点击进入详情

**结构**：

```
┌─────────────────────────────────────────────────┐
│  Kindle → Anki                 Inactive  v2.0  │  ← Inactive Badge
│                                                 │
│  Development         │  Runtime                │
