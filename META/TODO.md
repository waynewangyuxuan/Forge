...
- [ ] 0.4 Update MILESTONES/META.md - Mark M3 in-progress

### Section 1: Shared Layer - IPC Types
- [ ] 1.1 Add `spec:read` and `spec:save` channels to IPCChannelMap
**Tests:**
- [ ] 1.T1 TypeScript compile passes

### Section 2: Backend - Spec Use Cases
- [ ] 2.1 Create `read-spec.ts` - Read spec file content by versionId + file type
- [ ] 2.2 Create `save-spec.ts` - Save spec file content
- [ ] 2.3 Create barrel export `index.ts`
- [ ] 2.4 Update use-cases index to export spec
**Tests:**
...
- [ ] 3.1 Create `spec.ipc.ts` with `spec:read` and `spec:save` handlers
- [ ] 3.2 Register handlers in `index.ts`
**Tests:**
...
- [ ] 4.1 Create generic `Tabs` component following design system
- [ ] 4.2 Create barrel export
- [ ] 4.3 Update primitives index if needed
**Tests:**
...
- [ ] 5.1 Install CodeMirror and react-markdown dependencies
- [ ] 5.2 Create `MarkdownEditor` with CodeMirror integration
- [ ] 5.3 Create `MarkdownPreview` with react-markdown
- [ ] 5.4 Style components to match warm industrial design
**Tests:**
...
- [ ] 6.1 Create `useUnsavedChanges` hook with React Router blocker + beforeunload
- [ ] 6.2 Create `SpecPage` with tabs, edit/preview toggle, save functionality
- [ ] 6.3 Update router to use actual SpecPage
**Tests:**
